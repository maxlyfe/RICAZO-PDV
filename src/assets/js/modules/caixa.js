...
class CaixaModule {
  ...
  abrirModalFechoCaixa() {
    const content = `
      <div class="card-header">
        <h3 class="card-title">🔐 Fechamento de Caixa (Contagem Cega)</h3>
  ...
  async processarFechoCaixa(event) {
    ...
    try {
      ...
      alert('❌ Erro no fechamento de caixa: ' + error.message);
      btn.disabled = false;
  ...
  imprimirTicket(venda, pagamentos, trocoTotal, subtotal, taxaValor, taxaPercent, isReimpressao = false) {
    ...
    const html = `
      ...
      <div class="ticket-footer">
        <div class="ticket-thanks">OBRIGADO PELA PREFERÊNCIA!</div>
